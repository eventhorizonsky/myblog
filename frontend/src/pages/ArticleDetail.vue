...
import { ArrowLeft, ExternalLink, Calendar, ChevronLeft, ChevronRight } from "lucide-vue-next";

const props = defineProps<{ id: string }>();
const router = useRouter();

const allArticles = computed(() => loadAllArticles());
const article = computed(() => allArticles.value.find((a) => a.id === props.id));

// 画廊模式
const showGallery = computed(() => article.value?.type === "blocks" && (article.value?.images?.length || 0) > 0);
...
<template>
  <div v-if="article" class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
    <button
    ...
    <div class="mb-8">
      <div class="flex items-center gap-2 mb-3">
        <Badge variant="outline" :class="['text-xs px-1.5 py-0', CATEGORY_CONFIG[article.category]?.color || '']">
  ...
  <div v-else class="max-w-3xl mx-auto px-4 sm:px-6 py-16 text-center text-muted-foreground">
    <p class="text-lg mb-2">文章未找到</p>
    <p class="text-sm">ID: {{ id }}</p>
    <button @click="router.push('/articles')" class="mt-4 text-primary hover:underline text-sm">返回文章列表</button>
  </div>
</template>
